...
#PARAMATERS TO SET
tournament_name = "superrugby"

def create_url(start_year, page):

    url = "http://www.scorespro.com/rugby-union/ajaxdata.php?country=world&comp=super-rugby&league=&season=" + str(
        start_year) + "&status=results&page=" + str(page)
...
def start_crawl(current_year, start_page, endyear):
    ...
    while current_year > endyear:
        while start_page < 10:
            url = create_url(current_year, start_page)
